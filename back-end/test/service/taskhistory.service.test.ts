...
test("given valid task and user; when getting finished tasks from user's taskhistory; then finished tasks are returned", async () => {
    ...
    //when:
    const result = await taskhistoryService.getAllFinishedTasksByUser(1, {
...
test('given unknkown userid; when getting finished tasks from takshistory; then error is thrown', async () => {
    ...
    //when:
    const testing = async () =>
        await taskhistoryService.getAllFinishedTasksByUser(505, {
            username: user.getUsername(),
            role: user.getRole(),
        });
    //then:
...
test('given invalid userId; when getting finished tasks from taskhistory, then error is thrown;', async () => {
    //when
    const testing = async () =>
        await taskhistoryService.getAllFinishedTasksByUser(0, {
            username: user.getUsername(),
            role: user.getRole(),
        });
    //then:
...
test('given user without history; when getting finished tasks from taskhistory, then error is thrown;', async () => {
    ...
    //when
    const testing = async () =>
        await taskhistoryService.getAllFinishedTasksByUser(1, {
            username: user.getUsername(),
            role: user.getRole(),
        });
    //then:
...
test('given role guest, when calling taskhistory, then unauthorizingerror', async () => {
    //given:
    userDb.getUserById = mockUserDbgetUserById.mockResolvedValue(user);
    //when:
    const testing = async () => {
    ...
    //then:
    expect(testing).rejects.toThrow(
...
test('given guest role, when addingTaskToTaskHistory, then unauthorizationerror is thrown', async () => {
    //when:
    const testing = async () => {
        await taskhistoryService.addFinishedTaskToHistoryByUser(1, 3, {
            username: user.getUsername(),
            role: 'guest',
        });
    };
